...
Public Class clsFileStorage
    ...
    Dim _FileStorageID, _AgreementID, _ReferenceNo, _FileName, _OriginalFileName, _Attachment, _FileCategoryID, _CustodianID, _ResponsibleID, _CabinetLocationID, _Remarks, _
    ...
    Public Function fnInsertFileStorage(ByVal Storage As clsFileStorage) As clsResult
        ...
        Try
            ...
            cmd.Parameters.AddWithValue("@OriginalFileName", Storage.OriginalFileName)
    ...
    Public Function fnUpdateFileStorage(ByVal Storage As clsFileStorage) As clsResult
        Dim Result As New clsResult()
        Try
            Dim cmd As SqlCommand = New SqlCommand("spUpdateFileStorage", con)
            con.Open()
            cmd.CommandType = CommandType.StoredProcedure
            cmd.Parameters.AddWithValue("@FileStorageID", Storage.FileStorageID)
            cmd.Parameters.AddWithValue("@ReferenceNo", Storage.ReferenceNo)
            cmd.Parameters.AddWithValue("@FileName", Storage.FileName)
            cmd.Parameters.AddWithValue("@OriginalFileName", Storage.OriginalFileName)
            cmd.Parameters.AddWithValue("@Attachment", Storage.Attachment)
    ...
    Public Function fnGetFileStorageByID(ByVal FileStorageID As String) As clsFileStorage
        Dim sp As String = "spGetFileStorageByID"
        Dim FileStorage As New clsFileStorage()
        Try
            con.Open()
            Using cmd As SqlCommand = New SqlCommand(sp, con)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("@FileStorageID", FileStorageID)

                Dim dr As SqlDataReader = cmd.ExecuteReader()
                While dr.Read()
                    FileStorage.ReferenceNo = dr.Item("ReferenceNo")
                    FileStorage.FileName = dr.Item("FileName")
                    FileStorage.OriginalFileName = dr.Item("OriginalFileName")
                    FileStorage.Attachment = dr.Item("Attachment")
                    FileStorage.FileCategoryID = dr.Item("FileCategoryID")
                    FileStorage.CustodianID = dr.Item("CustodianID")
                    FileStorage.ResponsibleID = dr.Item("ResponsibleID")
                    FileStorage.CabinetLocationID = dr.Item("CabinetLocationID")
                    FileStorage.Remarks = dr.Item("Remarks")
                    FileStorage.EffectiveDate = dr.Item("EffectiveDate")
                End While
                con.Close()
                Return FileStorage
            End Using
    ...
    Public Function fnSearchStorageFiles(ByVal EmployeeID As String, ByVal SearchText As String) As DataSet
